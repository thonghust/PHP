<?php
	ob_start();
	require_once("connect.php");
	require_once("ham.php");
	session_start();
	$tukhoa = $_SESSION['timkiem'];
	$tt = "Tin tức - sự kiện";
	$q = mysqli_query($con, "select * from timkiem where tenbaiviet like '%$tukhoa%' or noidung like '%$tukhoa%'");
	if($tukhoa == "")
	{
		$dem = 0;
	}
	else
	{
		$dem = mysqli_num_rows($q);
	}
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>Kết quả tìm kiếm</title>
<link rel="stylesheet" type="text/css" href="dd.css">
</head>
<body>
<form method="post" enctype="multipart/form-data">
<div style="width:1100px; height:1500px; background:#ffc; margin:auto" align="center">
<font style="color:#00F; font-weight:bold; font-size:16px; font-family:Tahoma, Geneva, sans-serif"><?php 
    if($_SESSION['tk'])
	{
		echo "Xin chào, " ?><font style="color:#F00; font-weight:bold; font-size:16px; font-family:Tahoma, Geneva, sans-serif"><?php echo $_SESSION['tk']. " "; ?></font>
     <font style="font-weight:bold; font-size:16px; font-family:Tahoma, Geneva, sans-serif; color:#60F">[</font><a href="dangxuat.php" style="color:#00F; text-decoration:none">Đăng xuất</a></font>
     <font style="font-weight:bold; font-size:16px; font-family:Tahoma, Geneva, sans-serif; color:#60F">]</font>
     <?php
	}
	 else
	{ ?>
    Vui lòng đăng nhập/đăng ký để có thể nhận quyền user
    <input type="submit" name="dn" value="Đăng nhập">
	<input type="submit" name="dk" value="Đăng ký"> <?php } ?></font>
<img src="img/banner3.jpg" width="1100px" height="200px"><br>
<div class="menu">
<div class="giua1">
<ul>
<?php
	 $m = mysqli_query($con, "select * from topic where topic != '$tt'");
	 while($n = mysqli_fetch_array($m))
	 { ?>
     <li><a href="dongy.php?id=<?php echo $n['id']; ?>"><?php echo $n['topic']; ?></a></li>
     <?php } ?>
     <li style="background:#00F"><table style="background:#00F"><tr><td><textarea rows="1" cols="16" name="search1">Tìm kiếm...</textarea></td><td><input type="submit" name="search2" value="Tìm"></td></tr></table></li>
</ul>
</div>
</div>
<font style="font-family:Tahoma, Geneva, sans-serif; font-style:italic; font-size:20px; padding-left:10px; font-weight:bold"><a href="home.php" style="color:#00F; text-decoration:none">Trang chủ</a> »  <a href="#" style="color:#00F; text-decoration:none">Tìm kiếm tin tức</a></font><br><br>
<font style="font-family:Tahoma, Geneva, sans-serif; font-style:italic; font-size:20px; padding-left:10px; font-weight:bold">Tìm kiếm bài viết</font><br><br>
<?php
	if(isset($_POST['ok']))
	{
		if($_POST['sapxep'] == "Xem nhiều nhất")
		{
			$_SESSION['sort'] = "xem";
		}
		if($_POST['sapxep'] == "Like nhiều nhất")
		{
			$_SESSION['sort'] = "like";
		}
		if($_POST['sapxep'] == "Heart nhiều nhất")
		{
			$_SESSION['sort'] = "heart";
		}
		if($_POST['sapxep'] == "Theo ngày tạo")
		{
			$_SESSION['sort'] = "ngaytao";
		}
		if($_POST['sapxep'] == "Sắp xếp theo")
		{
			$_SESSION['sort'] = "ngaytao";
		}
	}
	?>
<table>
<tr>
<td><select name="sapxep">
    	<option value="<?php $_SESSION['sort']; ?>"<?php if($_SESSION['sort'] == "ngaytao") echo "selected"; ?>>Sắp xếp theo</option>
    	<option value="<?php $_SESSION['sort']; ?>"<?php if($_SESSION['sort'] == "ngaytao") echo "selected"; ?>>Theo ngày tạo</option>
        <option value="<?php $_SESSION['sort']; ?>"<?php if($_SESSION['sort'] == "xem") echo "selected"; ?>>Xem nhiều nhất</option>
        <option value="<?php $_SESSION['sort']; ?>"<?php if($_SESSION['sort'] == "like") echo "selected"; ?>>Like nhiều nhất</option>
        <option value="<?php $_SESSION['sort']; ?>"<?php if($_SESSION['sort'] == "heart") echo "selected"; ?>>Heart nhiều nhất</option>
    </select></td>
    
<td><input type="text" name="txttimkiem" value="<?php echo $_SESSION['timkiem']; ?>"></td>
<td><img src="img/search.png" width="25px" height="25px"></td>
</tr>
</table>
<input type="submit" name="ok" value="Tìm kiếm bài viết"></font><br><br>
<font style="font-family:Tahoma, Geneva, sans-serif; font-style:italic; font-size:20px; padding-left:10px; font-weight:bold">Có "<font color="#FF0000"><?php echo $dem; ?></font>" kết quả tìm kiếm cho từ khóa "<font color="#FF0000"><?php echo $_SESSION['timkiem']; ?></font>"</font><br><font style="font-family:Tahoma, Geneva, sans-serif; font-style:italic; font-size:20px; padding-left:10px; font-weight:bold">Sắp xếp theo: <font color="#FF0000"><?php 
	if($_SESSION['sort'] == "xem") echo "Xem nhiều nhất";
	if($_SESSION['sort'] == "like") echo "Like nhiều nhất";
	if($_SESSION['sort'] == "heart") echo "Thả tim nhiều nhất";
	if($_SESSION['sort'] == "ngaytao") echo "Ngày tạo";
 ?></font></font><br>
<table cellpadding="5" width="800px">
<?php
		date_default_timezone_set('Asia/Ho_Chi_Minh');
		$tukhoa = $_SESSION['timkiem'];
		if(empty($tukhoa)) echo "<script>alert('Nội dung không được để trống')</script>";
		else{
				if($_SESSION['sort'] == "xem")
				{
					$q = mysqli_query($con, "select * from timkiem where tenbaiviet like '%$tukhoa%' or noidung like '%$tukhoa%' order by luotxem desc");
				}
				else if($_SESSION['sort'] == "like")
				{
					$q = mysqli_query($con, "select * from timkiem where tenbaiviet like '%$tukhoa%' or noidung like '%$tukhoa%' order by thich desc");
				}
				else if($_SESSION['sort'] == "heart")
				{
					$q = mysqli_query($con, "select * from timkiem where tenbaiviet like '%$tukhoa%' or noidung like '%$tukhoa%' order by thatim desc");
				}
				else if($_SESSION['sort'] == "ngaytao")
				{
					$q = mysqli_query($con, "select * from timkiem where tenbaiviet like '%$tukhoa%' or noidung like '%$tukhoa%'");
				}
				$dem = mysqli_num_rows($q);
				if(mysqli_num_rows($q) == 0)
				{
					echo "<script>alert('Không tìm thấy bài viết nào')</script>";
				}
				else
				{
					while($num = mysqli_fetch_array($q))
					{ ?>
		<tr>
        <td>
        <a href="baiviet.php?id=<?php echo $num['id']; ?>"><img style="float:left" width="100px" height="100px" src="img/<?php echo $num['anh'];?> "></a><a href="baiviet.php?id=<?php echo $num['id']; ?>" style="text-decoration:none"><font style="color:#00F; font-weight:bold; font-size:16px; font-family:Tahoma, Geneva, sans-serif"><?php echo $num['tenbaiviet']; ?></font></a><br><font style=" font-size:14px; font-family:Tahoma, Geneva, sans-serif"><?php $noidung = substr( $num['noidung'],0,300); echo $noidung."..."; ?></font>
      <a href="dongy.php?id=<?php echo $num['idtopic']; ?>" style="text-decoration:none"><font style="color:#00F; font-weight:bold; font-size:12px; font-family:Tahoma, Geneva, sans-serif"> - <?php echo $num['topic']; ?> </font></a><br>
<font style=" font-size:12px; font-family:Tahoma, Geneva, sans-serif; font-style:italic;"><?php echo $num['time']; ?> | Người tạo: <?php echo $num['nguoitao']; ?> | <?php echo $num['luotxem']; ?> lượt xem | <?php echo $num['thich']; ?> lượt thích | <?php echo $num['thatim']; ?> lượt thả tim
<?php
if($_SESSION['tk'] == "admin")
{ ?>
| Tùy chọn: <font style=" padding-right:1px; font-size:12px"><a style="text-decoration:none; font-weight:bold; color:#00F" href='xoabai.php?xoa="<?php echo $num['id'];?>"' onClick="if(confirm('Bạn có thực sự muốn xóa?')) return true; else return false;">Xóa</a></font> • <font style=" padding-right:1px; font-size:12px"><a style="text-decoration:none; font-weight:bold; color:#00F" href='suabai.php?sua="<?php echo $num['id'];?>"'>Sửa</a></font> <?php } ?></font>
<div style="clear:both"><?php } ?></div>
		</tr>            
			  <?php }
				}
?>
</td>
</table>
</div>
</form>
</body>
</html>
<?php 
	if(isset($_POST['dn']))
	{
		header('location:dangnhap.php');
	}
	if(isset($_POST['dk']))
	{
		header('location:dangky.php');
	}
	if(isset($_POST['themcd']))
	{
		header('location:thembai.php');
	}
	if(isset($_POST['ok']))
	{
		$_SESSION['timkiem'] = $_POST['txttimkiem'];
		header('location:timkiem.php');
	}
	ob_end_flush();
?>